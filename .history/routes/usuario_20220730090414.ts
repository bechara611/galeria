...
import { RecuperarPassword, UsuarioDelete, UsuarioGet, UsuarioPost, UsuarioPut, UsuariosGet } from '../controllers/usuario';
...
router.put('/', [
    ...
    check('password', 'Password must have a value').not().isEmpty(),
    check('correo', 'email must have a correct value').isEmail(),
    check('nueva_password', 'New password must have a value').not().isEmpty(),
    comprobarCampos,
], UsuarioPut)

router.post('/recuperar/',[
    check('email', 'email must have a correct value').isEmail(),
    comprobarCampos,
],RecuperarPassword)
