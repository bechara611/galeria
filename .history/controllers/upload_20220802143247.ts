...
const postUpload = async (req: Request, res: Response) => {
    try {
        ...
        if (Object.keys(req.files).length != 1){
            for (let imagen in req.files.imagenes) {
                const { tempFilePath } = req.files.imagenes[imagen] as fileUpload.UploadedFile;
                const respuesta = await cloudinary.v2.uploader.upload(tempFilePath)
                // const respuesta = await cloudinary.v2.uploader.upload(req.files.imagenes[imagen].tempFilePath)
                    .then(async (data) => {
